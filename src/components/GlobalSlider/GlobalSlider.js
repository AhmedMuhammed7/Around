...
const GlobalSlider = ({ elements, elementsMapping }) => {
  ...
  let slides = (elementsLength)=> elements.reduce((acc, elm, index) => {
    const chunkIndex = Math.floor(index / elementsLength)
    if (!acc[chunkIndex]) acc[chunkIndex] = []
    acc[chunkIndex].push(elm)
    return acc
  }, [])

  const slidesList = slides(elementsNumber).map((slide, index) => (
    <div
      key={index}
  ...
  return (
    <Slider {...settings} className="glocal-slider">
      {slidesList}
    </Slider>
  )
}
